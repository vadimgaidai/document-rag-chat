...
const DOC_SEED_STEP = 7_919
const WORD_TARGETS = [26_000, 32_000, 41_000, 54_000, 302_000] as const
const WORD_BOUNDS = { regular: [25_000, 60_000], large: [300_000, Infinity] } as const
...
const STRESS_SEED_OFFSET = 6 * DOC_SEED_STEP
...
class Random {
  ...
  pick<T>(values: readonly T[]): T {
    return values[Math.floor(this.float() * values.length)]
  }
}
...
const codeBlock = (random: Random, pack: TPack): TBlockDraft => {
  ...
  return { markdown: ["```" + pack.codeLang, ...body, "```"].join("\n"), kind: "other" }
}
...
const buildBlocks = (random: Random, pack: TPack, wordTarget: number, docIndex: number) => {
  ...
  const sections = [...pack.sections, ...SHARED_SECTIONS]
  const subsections = [...pack.subsections, ...SHARED_SUBSECTIONS]
...
const factsOf = (docIndex: number) => PLANTED_FACTS.filter((fact) => fact.doc === docIndex)

const plantInto = (blocks: TBlockDraft[], docIndex: number) => {
  const insertions = factsOf(docIndex).flatMap((fact) => [
    { position: fact.position, order: `${fact.id}-fact`, block: factBlock(fact) },
    {
...
const render = (file: string, blocks: readonly TBlockDraft[]) => {
  const lines: string[] = []
  ...
  return { text, words: countWords(text), ranges }
}

const rangeOf = (ranges: ReadonlyMap<string, TRange>, tag: string, file: string): TRange => {
...
for (const [docIndex, pack] of PACKS.entries()) {
  ...
  if (docIndex === INJECTION_DOC) {
    injection = { file, ...rangeOf(ranges, "injection", file) }
  }
...
if (process.argv.includes(STRESS_FLAG)) {
  ...
  const { text, words } = render(STRESS_FILE, blocks)
